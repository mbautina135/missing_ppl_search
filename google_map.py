...
def render_google_map(zones, pins, api_key, map_placeholder):
    ...
    Args:
        ...
        map_placeholder (streamlit.delta_generator.DeltaGenerator): Streamlit placeholder to render the map.
    """
    import json
    ...
    <script>
          function initMap() {{
            const map = new google.maps.Map(document.getElementById("map"), {{
              center: {{ lat: 37.776, lng: -122.4194 }},
              zoom: 12,
            }});
            ...
            pins.forEach((pin) => {{
                const marker = new google.maps.Marker({{
                    ...
                    title: pin.label,
                    icon: {{
                        url: pin.icon || undefined,  // Use custom icon if provided
                        scaledSize: new google.maps.Size(40, 40)  // Fixed size for icons
                    }}
                }});
    ...
    with map_placeholder:
        st.components.v1.html(map_html, height=430)  # Reduced height from 600 to 400
